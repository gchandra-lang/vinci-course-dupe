...
_DAY02_ROOT = Path(__file__).resolve().parent


def resolve_day02_path(path: Path, *, day02: Path | None = None) -> Path:
    """
    Resolve Day 2 lab-relative paths (e.g. ../lab-04/plan.json).

    README paths assume cwd inside a lab folder; when run from repo root,
    ../lab-04/... would miss — re-anchor paths that start with lab-XX/ under day-02.
    """
    day02 = (day02 or _DAY02_ROOT).resolve()
    if path.is_absolute():
        return path.resolve()
    ...
    while i < len(parts) and parts[i] == "..":
        i += 1
    rest = parts[i:]
    if rest and str(rest[0]).startswith("lab-"):
        return (day02 / Path(*rest)).resolve()
