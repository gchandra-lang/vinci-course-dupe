...
Usage (from repo root):
  ...
  python course/student/day-02/lab-00/lab00_day2_readiness.py en6
  python course/student/day-02/lab-00/lab00_day2_readiness.py --validate-scenario my_team_scenario.json
  python course/student/day-02/lab-00/lab00_day2_readiness.py --write-scenario my_team_scenario.json
"""
...
DAY01 = REPO_ROOT / "course" / "student" / "day-01"
...
DAY1_REQUIRED = (
    ...
    "go2_motion_helpers.py",
    "go2_network_helpers.py",
)

DAY2_PREREQ = (
    "lab-02/lab02_obstacle_avoid_intro.py",
...
def check_day1_artifacts() -> bool:
    ok = True
    if not DAY01.is_dir():
        _fail(f"Day 1 folder missing: {DAY01}")
        return False
    for rel in DAY1_REQUIRED:
        path = DAY01 / rel
        if path.is_file():
            _ok(f"day-01/{rel}")
        else:
...
def check_day2_early_labs() -> bool:
    ...
    ok = True
    day02 = DAY02_LAB00.parent
    for rel in DAY2_PREREQ:
        path = day02 / rel
        if path.is_file():
            _ok(f"day-02/{rel}")
        else:
            _fail(f"missing day-02/{rel}")
            ok = False
    return ok
...
def main() -> int:
    ...
    print("  Next:")
    print("    1. Copy scenario: python course/student/day-02/lab-00/lab00_day2_readiness.py \\")
    print("         --write-scenario my_team_scenario.json")
    print("    2. Edit checkpoints and limits; validate:")
    print("         python course/student/day-02/lab-00/lab00_day2_readiness.py \\")
    print("         --validate-scenario my_team_scenario.json")
    print("    3. Wire Go2 and re-run with interface:")
    print("         python course/student/day-02/lab-00/lab00_day2_readiness.py en6")
