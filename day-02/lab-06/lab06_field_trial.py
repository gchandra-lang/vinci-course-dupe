#!/usr/bin/env python3
"""
Lab 6 (Day 2) — Field trial: tuned plan + Lab 5 patrol run + field_test.md report.

Usage:
  # 1) Tune plan (no robot)
  python course/student/day-02/lab-06/lab06_tune_plan.py \\
    ../lab-04/patrol_plan.cone_course.json --out course/student/day-02/lab-04/patrol_plan.tuned.json \\
    --leg 1 --dyaw 0.7 --leg 2 --dx 0.25

  # 2) Field trial run
  python course/student/day-02/lab-06/lab06_field_trial.py en6 -y \\
    --plan course/student/day-02/lab-04/patrol_plan.tuned.json \\
...
_DAY02_HELPER = Path(__file__).resolve().parents[1]
if str(_DAY02_HELPER) not in sys.path:
    sys.path.insert(0, str(_DAY02_HELPER))

from go2_patrol_helpers import resolve_day02_path  # noqa: E402

_REPO = Path(__file__).resolve().parents[4]
_LAB06 = Path(__file__).resolve().parent
_DAY02 = _LAB06.parent
_LAB05_RUNNER = _DAY02 / "lab-05" / "lab05_patrol_runner.py"
_VALIDATOR = _DAY02 / "lab-01" / "lab01_validate_run_folder.py"
_TEMPLATE = _LAB06 / "field_test.template.md"
_DEFAULT_PLAN = _DAY02 / "lab-04" / "patrol_plan.tuned.json"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Tuned patrol_plan.json (create with lab06_tune_plan.py)",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
    ...
    plan_path = resolve_day02_path(args.plan)
    if not plan_path.is_file():
        print(f"ERROR: plan not found: {plan_path}")
        print("  Create with lab06_tune_plan.py first.")
        return 1
    ...
    if args.dry_run:
        print("[dry-run] Would run lab05_patrol_runner.py then write field_test.md")
    ...
    if args.skip_patrol:
        ...
        if not _LAB05_RUNNER.is_file():
            print(f"ERROR: missing {_LAB05_RUNNER}")
            return 1
        cmd = [
            sys.executable,
            str(_LAB05_RUNNER),
